...
namespace RaidBattlesBot.Handlers
{
  ...
  public class ReplyHandler : IMessageHandler
  {
          ...
          if (button.CallbackData is { } buttonCallbackData && buttonCallbackData.Split(":", 3) is {} callbackDataParts)
          {
            if (callbackDataParts.Length > 1 && callbackDataParts[0] == VoteCallbackQueryHandler.ID)
            {
              if (PollEx.TryGetPollId(callbackDataParts[1], out var pollId, out _))
              {
                ...
                context.context = await ProcessMessage(message, poll, cancellationToken); 
    ...
    public async Task<PollMessage> ProcessMessage(Message message, [NotNull] Poll poll, CancellationToken cancellationToken = default)
    {
      var fromUserId = message.From?.Id;
      var participants = poll.Votes.Select(_ => _.UserId).ToHashSet();
      if (fromUserId is not { } userId || !(participants.Contains(userId) || myBot.BotId == fromUserId))
        return null; // do not process replies from strangers
      ...
      foreach (var pollVote in poll.Votes)
      {
        ...
        if (voteUserId == fromUserId || voteUserId == message.Chat.Id) continue; // do not notify itself
        ...
        try
        {
          ...
          var fm = await bot.ForwardMessageAsync(voteUserId, message.Chat, message.MessageId,
      ...
      return new PollMessage(message)
      {
        BotId = myBot.BotId,
